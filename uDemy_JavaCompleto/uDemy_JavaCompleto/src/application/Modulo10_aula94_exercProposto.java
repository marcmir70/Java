...
import entities.Modulo10_Empregado;

public class Modulo10_aula94_exercProposto {
	...
	public static void main(String[] args) {
		...
		List<Modulo10_Empregado> list = new ArrayList<>();
...
//		List<double> empSalary = new ArrayList<>(); erro: precisa declarar dimensões?!?
		...
		for (int i=0; i<employees; i++) {
			...
			while ( ! validId ) {
				...
				if ( i > 0 ) {
					validId = true;
					for ( Modulo10_Empregado emp : list ) { // for (int j=0; j<i, j++) {
			...
			list.add(new Modulo10_Empregado(id, name, salary));
		}
		...
		for ( Modulo10_Empregado emp : list ) {
